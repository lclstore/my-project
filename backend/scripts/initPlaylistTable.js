...
async function initPlaylistTable() {
  try {
    console.log('🚀 开始初始化Playlist表...');

    // 读取SQL文件
    const sqlFilePath = path.join(__dirname, '../sql/create_playlist_table.sql');
    ...
    // 验证playlist表
    await verifyTable('playlist');

    // 验证playlist_music表
    await verifyTable('playlist_music');

    console.log('\n✅ Playlist表初始化完成！');

  } catch (error) {
    console.error('❌ 初始化Playlist表失败:', error);
    throw error;
  }
}
...
module.exports = { initPlaylistTable };
