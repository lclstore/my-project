...
async function initPlanNameSettingsTable() {
    try {
        console.log('🚀 开始初始化PlanNameSettings表...');

        // 读取SQL文件
        const sqlFilePath = path.join(__dirname, '../sql/create_plan_name_settings_tables.sql');
        ...
        // 验证plan_name_settings表
        await verifyTable('plan_name_settings');
        
        // 验证plan_name_settings_rule表
        await verifyTable('plan_name_settings_rule');

        console.log('\n✅ PlanNameSettings表初始化完成！');

    } catch (error) {
        console.error('❌ 初始化PlanNameSettings表失败:', error);
        throw error;
    }
}
...
module.exports = { initPlanNameSettingsTable };
