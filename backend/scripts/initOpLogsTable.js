...
async function initOpLogsTable() {
    try {
        console.log('🚀 开始初始化OpLogs表...');

        // 读取SQL文件
        const sqlFilePath = path.join(__dirname, '../sql/create_op_logs_table.sql');
        ...
        // 验证op_logs表
        await verifyTable('op_logs');

        console.log('\n✅ OpLogs表初始化完成！');

    } catch (error) {
        console.error('❌ 初始化OpLogs表失败:', error);
        throw error;
    }
}
...
module.exports = { initOpLogsTable };
