...
async function initPlanReplaceSettingsTable() {
  try {
    console.log('🚀 开始初始化PlanReplaceSettings表...');

    // 读取SQL文件
    const sqlFilePath = path.join(__dirname, '../sql/create_plan_replace_settings_table.sql');
    ...
    // 验证plan_replace_settings表
    await verifyTable('plan_replace_settings');

    // 验证plan_replace_settings_rule表
    await verifyTable('plan_replace_settings_rule');

    // 验证plan_replace_settings_workout表
    await verifyTable('plan_replace_settings_workout');

    console.log('\n✅ PlanReplaceSettings表初始化完成！');

  } catch (error) {
    console.error('❌ 初始化PlanReplaceSettings表失败:', error);
    throw error;
  }
}
...
module.exports = { initPlanReplaceSettingsTable };
