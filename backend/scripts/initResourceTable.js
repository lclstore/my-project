...
async function initResourceTable() {
    try {
        console.log('🚀 开始初始化Resource表...');

        // 读取SQL文件
        const sqlFilePath = path.join(__dirname, '../sql/create_resource_table.sql');
        ...
        // 验证resource表
        await verifyTable('resource');

        console.log('\n✅ Resource表初始化完成！');

    } catch (error) {
        console.error('❌ 初始化Resource表失败:', error);
        throw error;
    }
}
...
module.exports = { initResourceTable };
