...
async function initTemplateTable() {
  try {
    console.log('🚀 开始初始化Template表...');

    // 读取SQL文件
    const sqlFilePath = path.join(__dirname, '../sql/create_template_table.sql');
    ...
    // 验证template表
    await verifyTable('template');

    // 验证template_unit表
    await verifyTable('template_unit');

    console.log('\n✅ Template表初始化完成！');

  } catch (error) {
    console.error('❌ 初始化Template表失败:', error);
    throw error;
  }
}
...
module.exports = { initTemplateTable };
